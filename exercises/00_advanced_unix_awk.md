...
# we need to first combine the two files to get the missing data proportion and group information in a single file. As they are ordered the same way, we can just combine them.
...
awk '{missing[$7]+=$5; count[$7]++}END{for(g in missing){print g,missing[g]/count[g]}}' martin2019.combined  | column -t
...
awk '{print $0"\t"$1"_"$7}' martin2019.combined | column -t

# replace the fifth column by this new combined individual and group information
awk '{$5=$1"_"$7; print $0}' martin2019.combined | head | column -t
