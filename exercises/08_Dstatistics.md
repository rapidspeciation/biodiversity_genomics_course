...
```shell
# prepare SETS file
cp ~/biodiversity_genomics_course/datamartin2019/martin2019.info .
sed 's/Hnum.bsl.bra$/Outgroup/' martin2019info | grep -v "ind"  > Melpomene.sets.txt
...
```shell
Dsuite Dtrios mel_tim_num.snps.vcf Melpomene.sets.txt
```
...
If we had more trios, we could parallelize the analysis using `DtriosParallel`
```shell
# calculate Dstats
DtriosParallel \
    --cores 8 \
    Melpomene.sets.txt \
    mel_tim_num.snps.vcf
```
